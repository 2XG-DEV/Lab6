...
import com.example.lab6.models.Enrollment;
...
public class EnrollmentsJDBCRepo implements ICrudRepo<Enrollment> {
    ...
    public Enrollment create(Enrollment obj){
        ...
        try {
            ...
            String query1 = "insert into enrolls(courseID,studentId) VALUES (?,?)";
    ...
    public Enrollment update(Enrollment obj){
        ...
        try {
            Connection conn = DriverManager.getConnection(url,user,password);
            String query1 = "update university.enrolls set studentId=? , courseID=?  where studentId=? AND courseID=?";
            PreparedStatement myStatement = conn.prepareStatement(query1);
    ...
    public List<Enrollment> getAll(){
        ...
        try {
            ...
            ResultSet resultSet = myStatement.executeQuery("SELECT * FROM university.enrolls");
    ...
    public void delete(Enrollment obj){
        ...
        try {
            ...
            String query1 = "delete from university.enrolls where courseID = ? AND studentId = ?";
            PreparedStatement myStatement = conn.prepareStatement(query1);
            myStatement.setInt(1,obj.getCourseId());
            myStatement.setInt(2,obj.getStudentId());
            myStatement.execute();
